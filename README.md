...
Verify with `/df:help` inside your chosen runtime.
...
```bash
# Claude Code
npx devflow-cc --claude --global   # Install to ~/.claude/
npx devflow-cc --claude --local    # Install to ./.claude/

# OpenCode (open source, free models)
npx devflow-cc --opencode --global # Install to ~/.config/opencode/

# Gemini CLI
npx devflow-cc --gemini --global   # Install to ~/.gemini/

# All runtimes
npx devflow-cc --all --global      # Install to all directories
```
...
If everything passes, you move on. If something's broken, you don't manually debug — you just run `/df:execute-phase` again with the fix plans it created.
...
When all phases are done, `/df:complete-milestone` archives the milestone and tags the release.
...
| Command | What it does |
|---------|--------------|
| `/df:add-phase` | Append phase to roadmap |
| `/df:insert-phase [N]` | Insert urgent work between phases |
| `/df:remove-phase [N]` | Remove future phase, renumber |
| `/df:list-phase-assumptions [N]` | See Claude's intended approach before planning |
| `/df:plan-milestone-gaps` | Create phases to close gaps from audit |

### Session

| Command | What it does |
|---------|--------------|
| `/df:pause-work` | Create handoff when stopping mid-phase |
...
- `/df:plan-phase --skip-research`
- `/df:plan-phase --skip-verify`
...
npx devflow-cc --opencode --global --uninstall

# Local installs (current project)
npx devflow-cc --claude --local --uninstall
