...
- `/devflow:add-todo Add auth token refresh` → title = "Add auth token refresh"
...
```bash
node ~/.claude/devflow/bin/df-tools.cjs commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
...
2. Add another todo
3. View all todos (/devflow:check-todos)
```
</step>
