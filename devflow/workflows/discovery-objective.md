...
NOTE: For comprehensive ecosystem research ("how do experts build this"), use /devflow:research-objective instead, which produces RESEARCH.md.
</purpose>
...
See ~/.claude/devflow/templates/discovery.md `<discovery_protocol>` for full protocol.
</source_hierarchy>
...
1. **Scope the discovery** using ~/.claude/devflow/templates/discovery.md:
...
Use ~/.claude/devflow/templates/discovery.md.
...
1. Discuss objective context (/devflow:discuss-objective [current-objective])
2. Create objective plan (/devflow:plan-objective [current-objective])
