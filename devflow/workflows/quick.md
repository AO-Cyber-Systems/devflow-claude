...
```bash
INIT=$(node ~/.claude/devflow/bin/df-tools.cjs init quick "$DESCRIPTION")
```
...
```bash
node ~/.claude/devflow/bin/df-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
