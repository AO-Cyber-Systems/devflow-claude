<purpose>
Create all phases necessary to close gaps identified by `/df:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/df:add-phase` per gap.
</purpose>
...
PHASES=$(node ~/.claude/devflow/bin/df-tools.cjs phases list)
...
```bash
node ~/.claude/devflow/bin/df-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/df:execute-phase {N}` — if plans already exist
...
- [ ] User knows to run `/df:plan-phase` next
</success_criteria>
