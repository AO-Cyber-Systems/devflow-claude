...
```bash
PHASE_INFO=$(node ~/.claude/devflow/bin/df-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ~/.claude/devflow/bin/df-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ~/.claude/devflow/bin/df-tools.cjs find-phase "${PHASE}" --raw)
```
