...
allowed-tools:
  ...
  - Bash(grep * .claude/devflow.local.md)
...
```bash
test -f .claude/devflow.local.md && echo "Loop is ACTIVE" || echo "No active loop"
```
...
```bash
TASK_FILE=$(grep '^task_file:' .claude/devflow.local.md 2>/dev/null | sed 's/task_file: *//' | sed 's/^"\(.*\)"$/\1/')
