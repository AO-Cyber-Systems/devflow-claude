...
@test "state file contains prompt" {
  ...
  grep -q "My test prompt" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "state file has valid YAML frontmatter" {
  ...
  head -1 "$TEST_DIR/.claude/devflow.local.md" | grep -q "^---$"

  # Check required fields exist
  grep -q "^active:" "$TEST_DIR/.claude/devflow.local.md"
  grep -q "^iteration:" "$TEST_DIR/.claude/devflow.local.md"
  grep -q "^max_iterations:" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "defaults to unlimited iterations (0)" {
  run_setup "Test"
  grep -q "^max_iterations: 0$" "$TEST_DIR/.claude/devflow.local.md"
}

@test "sets max iterations with --max-iterations" {
  run_setup --max-iterations 50 "Test"
  grep -q "^max_iterations: 50$" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "sets completion promise with --completion-promise" {
  ...
  grep -q 'completion_promise: "ALL DONE"' "$TEST_DIR/.claude/devflow.local.md"
}

@test "handles empty completion promise" {
  run_setup "Test"
  grep -q 'completion_promise: ""' "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "sets task file with --task-file" {
  ...
  grep -q 'task_file: "custom_tasks.json"' "$TEST_DIR/.claude/devflow.local.md"
}

@test "auto-detects feature_list.json" {
  echo '{"features": []}' > "$TEST_DIR/feature_list.json"
  run_setup "Test"
  grep -q 'task_file: "feature_list.json"' "$TEST_DIR/.claude/devflow.local.md"
}

@test "auto-detects prd.json" {
  echo '{"tasks": []}' > "$TEST_DIR/prd.json"
  run_setup "Test"
  grep -q 'task_file: "prd.json"' "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "concatenates multiple prompt words" {
  ...
  grep -q "Implement all the features now" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "generates default prompt when only task file exists" {
  ...
  [[ -f "$TEST_DIR/.claude/devflow.local.md" ]] || skip "Script requires prompt"
  grep -q "feature_list.json" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "includes active field set to true" {
  run_setup "Test"
  grep -q "^active: true$" "$TEST_DIR/.claude/devflow.local.md"
}

@test "includes started_at timestamp" {
  run_setup "Test"
  grep -q "^started_at:" "$TEST_DIR/.claude/devflow.local.md"
}

@test "includes use_subagents field" {
  run_setup "Test"
  grep -q "^use_subagents:" "$TEST_DIR/.claude/devflow.local.md"
}

@test "includes use_native_tasks field" {
  run_setup "Test"
  grep -q "^use_native_tasks:" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "includes instructions section" {
  run_setup "Test"
  grep -q "## Instructions" "$TEST_DIR/.claude/devflow.local.md"
}

@test "mentions subagents in instructions" {
  run_setup "Test"
  grep -q "trd-implementer" "$TEST_DIR/.claude/devflow.local.md" || \
  grep -q "Subagent" "$TEST_DIR/.claude/devflow.local.md" || \
  grep -q "subagent" "$TEST_DIR/.claude/devflow.local.md"
}
...
@test "handles all options together" {
  ...
  grep -q "^max_iterations: 50$" "$TEST_DIR/.claude/devflow.local.md"
  grep -q 'completion_promise: "ALL DONE"' "$TEST_DIR/.claude/devflow.local.md"
  grep -q 'task_file: "custom.json"' "$TEST_DIR/.claude/devflow.local.md"
  grep -q "Implement features" "$TEST_DIR/.claude/devflow.local.md"
}

@test "options can be in any order" {
  run run_setup "My prompt" --max-iterations 10 --completion-promise "DONE"
  [[ $status -eq 0 ]]
  grep -q "My prompt" "$TEST_DIR/.claude/devflow.local.md"
  grep -q "^max_iterations: 10$" "$TEST_DIR/.claude/devflow.local.md"
}
