...
<!-- BROKEN: I8 — a route guard with no denied state — expected code GUARD001.

     ONE edit of the positive control: the whole `guard-denied` state is deleted, while the
     route `project.conversations` keeps `guards: [member-of-workspace]`. The spec now declares
     a gate with no picture of what a denied reader sees — the aodex#517 class, where a screen
     exists and no one can say what happens when you are not allowed on it.

     `guard-denied` appears in NO control's `visible_in`, so deleting it trips neither CTRL006
     nor the behaviour-coverage model, and it is not one of §4.4's minimum states, so STATE003
     stays silent too. Exactly one code, as §17 requires.

     THE LINKAGE RULE this fixture pins (34-04-SUMMARY.md, "the guard -> denied-state linkage
     rule"; 34-05's nav graph draws its guard edge from the same rule): a route guard `G` names
     its denied state by ID, resolved against `states[].id` in order — `G`, then `G-denied`,
     then the canonical `guard-denied`. The resolved state must ALSO declare an `as:` identity:
     that is the field which makes it a denied state rather than a data state, and it is what
     labels the guard edge. Here `member-of-workspace` resolves to none of the three.
-->
