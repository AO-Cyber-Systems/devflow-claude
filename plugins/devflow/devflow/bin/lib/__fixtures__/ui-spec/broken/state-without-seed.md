...
<!-- BROKEN: I4 — a state with no `seed` — expected code STATE001.

     ONE edit of the positive control: the `seed: projects-3` line is deleted from the
     `error` state (states[3]). §4.4: every state has a seed — without it the e2e entrypoint
     has nothing to seed the surface with and the state can never be captured, so a spec that
     omits one declares a state no probe can ever reach.
-->
