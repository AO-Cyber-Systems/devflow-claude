...
routes:
  - id: project.conversations
    ...
    entry: [{control: rail.project.missing}, deeplink]                # >=1 required
...
<!-- BROKEN: I2 — an entry names a control this spec does not declare — expected code ROUTE003.

     ONE edit of the positive control: `entry[0]` of `project.conversations` names
     `rail.project.missing` instead of `rail.project.header`. The bare string `deeplink`
     beside it is a declared non-control entry and stays VALID (case I2e) — only
     `{control: ...}` entries are resolved.

     The named id is not prefixed by this surface (`projects-rail`), so it may live in
     another spec of the repo; W1b validates ONE spec with no repo scan, so the message
     carries `resolution: MISSING` rather than claiming the control does not exist.
-->
