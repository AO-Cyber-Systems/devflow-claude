...
patterns: [navigation/does-not-exist]
references:
...
<!-- BROKEN: I5 — a referenced pattern the catalogue does not carry — expected code PAT001.
     ...
     navigation/section-caption. §4.5 I5: every referenced pattern exists in the pinned
     eden-ui-flutter release.

     Reaching this code through the REAL binary needs a catalogue, because an UNREACHABLE
     catalogue is PAT000/MISSING and never PAT001 — the two are different facts and collapsing
     them turns "I could not look" into "I looked and it was fine". So the arm takes
     `--patterns __fixtures__/ui-spec/pattern-catalogue.json`; without the flag this fixture
     validates ok:true with one PAT000 MISSING row, which is the honest answer, not a pass.
     See 34-04-SUMMARY.md, "PAT000/HIT000 MISSING semantics".
-->
