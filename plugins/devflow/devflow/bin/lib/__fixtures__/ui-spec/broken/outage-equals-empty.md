...
states:
  ...
  - id: outage
    ...
    content: {must_show: ["unavailable", "Create a project"], must_not_show: []}
  - id: guard-denied
    seed: projects-3
    as: non-member
...
<!-- BROKEN: I4 — `outage.content.must_show` INTERSECTS `empty.content.must_show` —
     expected code STATE002.

     ONE edit of the positive control: the `outage` state's `content:` line. It now shows
     ["unavailable", "Create a project"] where `empty` shows ["Create a project"], so the two
     lists INTERSECT without being EQUAL.

     Why not equality, which is what this file's NAME says? The amended proposal states the
     rule as `outage.must_show ∩ empty.must_show = ∅` in BOTH §4.4 and §4.5 I4 — disjoint, not
     merely unequal. An equality-only implementation PASSES this fixture, and an outage then
     evidences itself with the emptiness sentence, which is exactly the confusion §4.4 exists
     to prevent. The fixture therefore carries the case that distinguishes the two readings;
     the weaker "equal" half is pinned by case I4b instead. The file name is the one the TRD's
     `files_modified` declares and is kept for that reason. See 34-04-SUMMARY.md, "STATE002:
     intersection, not equality".

     `must_not_show` is emptied in the SAME line so the fixture does not also declare a state
     that must show and must not show the same sentence — a contradiction no invariant in this
     engine reports, but one a reader would rightly stumble over.
-->
