...
<!-- BROKEN: I2 — a route with no `back` and no `root: true` — expected code ROUTE002.

     ONE edit of the positive control: the `back:` line is deleted from the route
     `project.conversations`, which does not declare `root: true`. The other route,
     `conversations.all`, keeps its `root: true` and so is still exempt — the exemption is
     what makes this fixture fail with ROUTE002 ALONE rather than twice.
-->
