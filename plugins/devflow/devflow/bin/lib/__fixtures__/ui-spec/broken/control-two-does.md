...
<!-- BROKEN: I3 — a control declaring BOTH a top-level `does` and a `behaviors[]` list —
     expected code CTRL001.

     ONE edit of the positive control: a `does` + `effect` pair is added to
     `rail.project.header` ALONGSIDE its three behaviours. §4.3 states the rule as EITHER
     one `does` + `effect` pair OR a `behaviors[]` list, so the statically-checkable
     violation is a control that declares both — two competing statements of what it does.

     The other reading of "two does" — a literally duplicated `does:` KEY — is caught one
     layer down by yaml-lite's duplicate-key refusal (34-01 case Y11a) and never reaches
     this validator. Both readings are therefore covered and neither is guessed at.
-->
