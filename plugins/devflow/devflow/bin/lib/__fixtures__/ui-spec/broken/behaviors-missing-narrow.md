...
<!-- BROKEN: I3 — the behaviours do not cover every `visible_in` state — expected code CTRL004.

     ONE edit of the positive control: B3 (`when: {viewport: narrow}`) is deleted while
     `narrow` STAYS in `visible_in`. B1 and B2 both require `viewport: desktop`, and the
     `narrow` state declares a 390x844 viewport — below NARROW_MAX_WIDTH (600) — so both
     (narrow, collapsed) and (narrow, expanded) are matched by no behaviour at all.
-->
