...
if command -v mise &>/dev/null; then
  ...
  if echo "$PATH" | grep -q "mise/shims" || grep -q "mise/shims" "${HOME}/.zshrc" "${HOME}/.bashrc" "${HOME}/.bash_profile" "${HOME}/.profile" 2>/dev/null; then
    ...
    echo -e "  Or run: ${GREEN}/devflow:install${NC}"
  fi
else
...
echo -e "${YELLOW}Mobile Tools (Capacitor):${NC}"
check_tool "capacitor-cli" "command -v cap || test -f node_modules/.bin/cap" "_" "npm i -D @capacitor/cli @capacitor/core" || true
check_tool "java" "java --version" "java" "mise use -g java@21" || true
if [[ "$OS" == "macos" ]]; then
  check_tool "xcode" "xcodebuild -version" "_" "xcode-select --install" || true
  check_tool "cocoapods" "pod --version" "_" "gem install cocoapods" || true
fi
echo ""

echo -e "${YELLOW}Desktop Tools (Electron):${NC}"
check_tool "electron-vite" "command -v electron-vite || test -f node_modules/.bin/electron-vite" "_" "npm i -D electron-vite" || true
check_tool "electron-builder" "command -v electron-builder || test -f node_modules/.bin/electron-builder" "_" "npm i -D electron-builder" || true
echo ""
...
if [[ $MISSING_COUNT -gt 0 ]]; then
  ...
  if [[ "$MISE_AVAILABLE" == "true" ]]; then
    echo -e "Run ${GREEN}/devflow:install${NC} to auto-install via mise."
  else
    echo -e "Install mise first, then run ${GREEN}/devflow:install${NC}"
  fi
  echo ""
fi
