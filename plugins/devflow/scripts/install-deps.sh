...
# Java (for Android / Capacitor)
echo "Java (mobile/Capacitor):"
JAVA_MAJOR=$(java --version 2>&1 | head -1 | sed 's/.*"\?\([0-9]\+\)\..*/\1/' || echo "0")
if command -v java &>/dev/null && [[ "$JAVA_MAJOR" -ge 21 ]]; then
  success "  java ($JAVA_MAJOR)"
else
  info "  Installing Java 21 via mise..."
  if mise use -g java@21 2>/dev/null; then
    mise reshim 2>/dev/null || true
    hash -r 2>/dev/null || true
    success "  java 21 installed"
  else
    warn "  java 21: mise install failed (install manually: mise use -g java@21)"
  fi
fi
...
echo -e "${BLUE}==================================${NC}"
echo ""
echo -e "Run ${GREEN}/devflow:doctor${NC} to verify all dependencies."
echo ""
