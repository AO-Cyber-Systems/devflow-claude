...
```bash
# Create a new TRD
/devflow-agent:trd create "User authentication flow"
/devflow-agent:trd create "API rate limiting" --priority 2 --effort medium

# List all TRDs
/devflow-agent:trd list

# View a specific TRD
/devflow-agent:trd view 001

# Update status
/devflow-agent:trd status 001 in_progress
/devflow-agent:trd status 001 complete
```
...
1. Create TRD: `/devflow-agent:trd create "Feature name"`
2. Edit TRD to fill in details
3. Start work: `/devflow-agent:trd status 001 in_progress`
4. Implement following the TRD's technical approach
5. Complete: `/devflow-agent:trd status 001 complete`
6. Sync features: `/devflow-agent:features sync`
7. Add to regression: `/devflow-agent:regression add TRD-001`
