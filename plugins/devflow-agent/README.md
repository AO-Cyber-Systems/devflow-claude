...
### Development Mode
```bash
claude --plugin-dir ./plugins/devflow-agent
```
...
/devflow-agent:autonomous Build the REST API --completion-promise "ALL DONE"

# With iteration limit (safety net)
/devflow-agent:autonomous Fix all bugs --max-iterations 100
...
### Other Commands
```bash
/devflow-agent:status    # Check current loop status
/devflow-agent:cancel    # Cancel active loop
/devflow-agent:doctor    # Check for required tools
/devflow-agent:install   # Auto-install missing tools (no sudo)
/devflow-agent:help      # Show help
```
...
```
┌─────────────────────────────────────────────────────────┐
│  1. /devflow-agent:autonomous "Build features"          │
│                         │                               │
│                         ▼                               │
│  2. Creates .claude/devflow-agent.local.md              │
...
├── commands/
│   ├── autonomous.md       # /devflow-agent:autonomous
│   ├── cancel.md           # /devflow-agent:cancel
│   ├── status.md           # /devflow-agent:status
│   └── help.md             # /devflow-agent:help
├── hooks/
