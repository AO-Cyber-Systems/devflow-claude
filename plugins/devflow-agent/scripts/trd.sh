...
cmd_create() {
  ...
  echo -e "  ${BLUE}/devflow-agent:features sync${NC} to update feature_list.json"
}
...
cmd_status() {
  ...
  if [[ "$new_status" == "complete" ]]; then
    ...
    echo "  /devflow-agent:features sync   # Update feature_list.json"
    echo "  /devflow-agent:regression add TRD-$id  # Add tests to regression suite"
  fi
}
