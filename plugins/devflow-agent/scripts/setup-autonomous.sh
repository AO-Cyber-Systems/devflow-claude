...
USAGE:
  /devflow-agent:autonomous [PROMPT...] [OPTIONS]
...
EXAMPLES:
  ...
  /devflow-agent:autonomous Implement all features in feature_list.json

  # With completion promise
  /devflow-agent:autonomous Build the API --completion-promise "ALL DONE"

  # With specific task file
  /devflow-agent:autonomous --task-file prd.json Implement user stories

  # With iteration limit (safety)
  /devflow-agent:autonomous Fix all bugs --max-iterations 50

MONITORING:
  cat .claude/devflow-agent.local.md    # View state
  grep '^iteration:' .claude/devflow-agent.local.md  # Current iteration

STOPPING:
  /devflow-agent:cancel                 # Cancel the loop
HELP_EOF
  exit 0
}
...
if [[ -z "$PROMPT" ]] && [[ -z "$TASK_FILE" ]]; then
  ...
  echo "" >&2
  echo "Examples:" >&2
  echo "  /devflow-agent:autonomous Implement all features" >&2
  echo "  /devflow-agent:autonomous --task-file prd.json" >&2
  echo "" >&2
  echo "For help: /devflow-agent:autonomous --help" >&2
  exit 1
fi
...
cat > .claude/devflow-agent.local.md <<EOF
---
active: true
iteration: 1
...
To cancel: /devflow-agent:cancel
To monitor: cat .claude/devflow-agent.local.md
