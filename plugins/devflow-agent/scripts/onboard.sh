...
cmd_verify_setup() {
  ...
  echo ""
  echo "Run '/devflow-agent:verify list' to see all checks"
  echo "Run '/devflow-agent:verify run' to execute verification"
}
...
EOF
    ...
    echo "Run '/devflow-agent:features sync' to update feature_list.json"
...
cmd_full() {
  ...
  echo "  2. Run '/devflow-agent:verify run' to test verification checks"
  echo "  3. Run '/devflow-agent:features sync' to generate feature list"
  echo "  4. Create new TRDs with '/devflow-agent:trd create <name>'"
  echo ""
}
