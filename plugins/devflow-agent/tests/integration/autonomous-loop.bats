...
@test "setup creates state, stop hook blocks, completion removes state" {
  ...
  grep -q "^iteration: 2$" "$TEST_DIR/.claude/devflow-agent.local.md"

  # Step 3: Simulate reaching max iterations
  # Update iteration to max
  sed -i.bak 's/^iteration: 2$/iteration: 5/' "$TEST_DIR/.claude/devflow-agent.local.md"
...
EOF
  ...
  sed -i.bak 's/^iteration: 1$/iteration: 10/' "$TEST_DIR/.claude/devflow-agent.local.md"
...
@test "stop hook maintains iteration count across calls" {
  ...
  grep -q "^iteration: 6$" "$TEST_DIR/.claude/devflow-agent.local.md"
}
...
@test "stop hook preserves prompt across iterations" {
  ...
  grep -q "My custom prompt for testing" "$TEST_DIR/.claude/devflow-agent.local.md"
}
