...
@test "increments iteration counter" {
  ...
  local new_iteration
  new_iteration=$(grep '^iteration:' "$TEST_DIR/.claude/devflow-agent.local.md" | sed 's/iteration: *//')
  [[ "$new_iteration" == "6" ]]
}
...
@test "atomic iteration update preserves other fields" {
  ...
  grep -q "max_iterations: 100" "$TEST_DIR/.claude/devflow-agent.local.md"
  grep -q 'completion_promise: "DONE"' "$TEST_DIR/.claude/devflow-agent.local.md"
