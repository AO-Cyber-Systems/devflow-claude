...
# State file location
STATE_FILE=".claude/devflow-agent.local.md"
DEVFLOW_DIR=".devflow"
    ...
    if [[ -f "$VERIFY_FILE" ]]; then
      ...
      if [[ -n "$LAST_RUN" ]] && [[ "$LAST_RUN" != "null" ]]; then
        ...
        if [[ "$VERIFY_FAILED" -gt 0 ]]; then
          ...
          VERIFICATION_MSG="Verification failed ($VERIFY_FAILED checks). Run /devflow-agent:verify run"
        fi
      else
        # No verification run recorded - suggest running
        VERIFICATION_MSG="Run /devflow-agent:verify run before completion"
      fi
    fi
    ...
    if [[ -f "$MANIFEST_FILE" ]]; then
      ...
      if [[ "$TOTAL_TESTS" -gt 0 ]]; then
        ...
        if [[ -n "$PASS_RATE" ]] && [[ "$PASS_RATE" != "100" ]] && [[ "$PASS_RATE" != "0" ]]; then
          VERIFICATION_PASSED=false
          VERIFICATION_MSG="${VERIFICATION_MSG:+$VERIFICATION_MSG | }Regression pass rate: ${PASS_RATE}%. Run /devflow-agent:regression run"
        fi
      fi
    fi
