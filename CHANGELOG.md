...
- **UI-eval bootstrap, setup, and executor gates find a monorepo `flutter/` package** — `flutter-ui-eval-bootstrap.cjs`, `flutter-ui-bootstrap.cjs`, `flutter-ui-setup.cjs`, and the executor's Flutter gates all assumed the package lived at `<repo>/pubspec.yaml`, reporting `flutter-not-detected` on monorepo consumers (eden-biz, aodex) where it's at `<repo>/flutter/pubspec.yaml`; a shared `resolveFlutterPackageDir()` now backs all four, and executor evidence paths are captured from `REPO_ROOT` once at the repo root rather than via `$OLDPWD`, which this harness's persistent-cwd/one-command-per-call model made unsound (1cea48a, 01d1f70, 19b7f48)
