...
- **Reference docs matched the shipped visual gate, not the pre-2.4.0 state** — `testing-strategy.md`'s Flutter visual/golden cell and `Out of scope` note still called the gate "deferred"; `ui-eval.md`, `ui-evaluator.md`, and `verifier.md` still told agents to capture Flutter web via `flutter run -d chrome`, which DWDS wedges into a silent blank page. Matrix cell now points at `df-tools verify flutter-ui-eval`; capture steps now build `flutter build web --release` and serve it statically
